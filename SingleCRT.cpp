...
// explicitly changing the number of levels, returns the previous number
...
// all moduli used). Using the optional fromLvl, numLvls params we compute the
// polynomial reduced modulo the product of only the ptimes in these levels.
...
void SingleCRT::SetIndex(long index, const ZZX &poly) {
  map[index] = poly;
}
